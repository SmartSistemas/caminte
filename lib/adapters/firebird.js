...
exports.initialize = function initializeSchema(schema, callback) {
    ...
    if (s.pool) {
        fb.pool(s.pool, options).get(function(err, client) {
            if (!err) {
...
FB.prototype.autoupdate = function(callback) {
    ...
    self.client.execute(
        'SELECT a.RDB$RELATION_NAME FROM RDB$RELATIONS a' +
                    ...
                    self.client.startTransaction(function(err, tr) {
                        ...
                        Object.keys(model.properties).forEach(function(field) {
                            ...
                            sql +=
                                ...
                                ' ' +
                                str +
                                (f.allowNull === false || f['null'] === false
                                    ? ' NOT NULL,'
                                    : ',') +
                                '\n';
                        });
...
FB.prototype.create = function(name, data, callback) {
    ...
    this.client.execute(sql, params, function(err, result) {
...
FB.prototype.findById = function findById(name, id, callback) {
    ...
    this.client.query(sql, id, function(err, result) {
        callback(err, result && result.length === 1 ? result[0] : undefined);
    });
};

FB.prototype.query = function(sql, callback) {
    if (typeof callback !== 'function') {
        throw new Error('callback deve ser uma função');
    }
    this.client.query(sql, (err, data) => callback(err, data));
};
...
FB.prototype.count = function count(name, callback, where) {
    ...
    this.client.execute(
        'SELECT count(*) FROM ' + quote(table) + buildWhere(where),
        params,
        function(err, result) {
...
FB.prototype.exists = function count(name, id, callback) {
    ...
    this.client.execute(sql, id, function(err, data) {
...
FB.prototype.updateOrCreate = function(name, data, callback) {
    ...
    this.client.execute(sql, params, function(err) {
        callback(err, data);
    });
};
